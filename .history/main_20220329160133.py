...
def DDQN():
    ...
    info = str(DQN.epoch) + "-" + str(DQN.train_interval) + "-" + str(DQN.update_interval) 
    + "-" + str(DQN.learning_rate) + "-" + str(DQN.gamma)
    ...
    for n_epi in range(DQN.epoch):
        ...
        if (high_score < score):
            high_score = score
            if os.path.isfile(('DQN_save_test/' + model_name)):
                os.remove(('DQN_save_test/' + model_name))
            model_name = 'model_' + str(n_epi) + '.pth'
            q.save(model_name)
        
        # Save model of highest goal
        if (env.now_time == env.lowest_time):
            if os.path.isfile(('DQN_save_test/' + model_name2)):
                os.remove(('DQN_save_test/' + model_name2))
            model_name2 = 'model2_' + str(n_epi) + '.pth'
            q.save(model_name2)
    
    file_name = 'DQN_graph/' + info + "_reward" + '.txt'
    with open(file_name,'w', encoding='UTF-8') as f:
        for i in reward_list:
            f.write(str(i) + '\n')
            
    file_name = 'DQN_graph/' + info + "_production_time" + '.txt'
    with open(file_name,'w', encoding='UTF-8') as f:
        for i in reward_list:
            f.write(str(i) + '\n')
...
def Deter(iter_num, model_option, machine_option):
    ...
    file_name = 'Deter_save/' + choose_model + choose_machine + '.txt'
    with open(file_name,'w', encoding='UTF-8') as f:
...
def Test(iter_num, test_file):
    ...
    path = 'DQN_save_test/' + test_file + '.pth'
    # Q network
    q = DQN.Qnet(len(env.reset()), len(env.choice)).to('cuda')
