...
def Graph_Deter():
    ...
    data_list = []
    data_dir = "Deter_data/"
    opt_list = ["Ri-0 3-0.txt", "C-1 3-0.txt", "C-1 3-0-3-0-1-2-2-1.txt", "R-1 3-0.txt"]
    for opt in opt_list:
        file_list_ = glob.glob(os.path.join(data_dir, opt))
        data = []
        for file_name in file_list_:
    ...
    data_name = ["Rigid model (rigide pattern)", "Rigid model (numerical pattern)", "Rigid model (random pattern)"
                "Circular model (rigid pattern)", "Circular model (numerical optimal)", "Rigid model (random pattern)"
                "Random model (Rigid pattern)", "Random model (numerical optimal)", "Rigid model (random pattern)"]
