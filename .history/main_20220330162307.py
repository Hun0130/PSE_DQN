...
def Graph_Deter():
    ...
    data_list = []
    data_dir = "Deter_data/"
    opt_list = ["Ri-0 3-0.txt", "Ri-1 3-0-3-0-1-2-2-1.txt", "Ri-0 Random.txt", 
                "C-1 3-0.txt", "C-1 3-0-3-0-1-2-2-1.txt", "C-1 Random.txt",
                "R-0 3-0.txt", "R-1 3-0-3-0-1-2-2-1.txt", "R-0 Random.txt"]
    for opt in opt_list:
        file_list_ = glob.glob(os.path.join(data_dir, opt))
        data = []
        for file_name in file_list_:
    ...
    data_name = ["Rigid (Rigid)", "Rigid (Numerical Optimal)", "Rigid (Random)"
                ...
                "Random (Rigid)", "Random (Numerical Optimal)", "Random (Random)"]
    
    ax.boxplot(data_list, notch=True)
    ax.legend(data_name, loc='center bottom', bbox_to_anchor=(0.5, 0.5))
