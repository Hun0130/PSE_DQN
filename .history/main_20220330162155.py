...
def Graph_Deter():
    ...
    opt_list = ["Ri-0 3-0.txt", "Ri-1 3-0-3-0-1-2-2-1.txt", "Ri-0 Random.txt", 
                "C-1 3-0.txt", "C-1 3-0-3-0-1-2-2-1.txt", "R-1 3-0.txt"]
    for opt in opt_list:
        file_list_ = glob.glob(os.path.join(data_dir, opt))
        data = []
        for file_name in file_list_:
    ...
    data_name = ["Rigid (Rigid)", "Rigid (Numerical Optimal)", "Rigid (Random)"
                "Circular (Rigid)", "Circular (Numerical Optimal)", "Rigid model (Random)"
                "Random (Rigid)", "Random (Numerical Optimal)", "Rigid (Random)"]
