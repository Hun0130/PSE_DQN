...
class factory(): 
    ...
    # 학습에 사용될 state를 생성
    def state_maker_changed(self, line_state, time_state, pattern_df, max_buffer): 
        # 결과를 저장할 list
        state = []
        # Machine state 확인
        idx = 0
        ...
        time_idx = 0
        # Get all stock
        # for stock in self.stock.items():
        #     state.append(stock[1])
        
        # 각 machine's current timer 
        for machine_idx in line_state[idx]:
            if machine_idx[0] == 'E':
                state.append(0)

            # machine state is not empty
            else:
                pattern = machine_idx[0][1]
                # "timer"에 적힌 시간을 저장
                time = machine_idx[1]
        ...
        idx = 1
        # 각 machine의 buffer를 확인
        for machine_idx in range(len(line_state[idx]) - 2):
            for buffer_idx in range(max_buffer[machine_idx]):
                # 버퍼에 제품이 존재할 시, timer를 result에 입력
                if buffer_idx < len(line_state[idx][machine_idx]):
                    model = line_state[idx][machine_idx][buffer_idx][0]
                    pattern = line_state[idx][machine_idx][buffer_idx][1]
                    # model, pattern에 해당하는 df에서 machine별 cycle time을 찾아서 저장
                    model_idx = self.find_model_index(model, pattern_df[pattern])
                    state.append(pattern_df[pattern][model_idx][1][machine_idx + 1][4])
    ...
    def check_empty(self):
        ...
        for buffer_idx in range(len(self.line_state[1]) - 1):
    ...
    def put(self, model, pattern):
        # 모델과 패턴, 현재 시간을 라인에 넣어줌 (없었다면 'E')
        self.line_state[0][0][0] = [model, pattern, self.now_time, 0, 0]  
    ...
    # ENV의 1 STEP : Machine1이 비워질때 or Product model
    def step(self, opt = 0):
        reward = 0
        # Machine 1이 빌 때 까지
        while ((self.line_state[0][0][0] != 'E') or (self.total_stock() == 0)):
            for machine_idx in range(len(self.line_state[0]) -1, -1, -1):
                    ...
                    if machine_idx != 0:
                        ...
                        # Starvation 발생 (전 버퍼가 비어있어서 진행 불가능)
                            ...
                            if self.line_state[0][machine_idx][0] != 'E':
                                self.starvation_time[machine_idx] += 1
                                self.line_state[0][machine_idx][0][3] += 1
                ...
                # Machine의 제품의 동작이 끝난 경우
                    ...
                    if len(self.line_state[1][machine_idx]) < self.maxbuffer[machine_idx]:
                        ...
                        if machine_idx == (len(self.line_state[0]) - 1):
                            ...
                            self.patterned_df, self.line_state[0][machine_idx][0][2], self.now_time, self.line_state[0][machine_idx][0][3],
                            self.line_state[0][machine_idx][0][4])
                            ...
                            if self.check_empty() and self.total_stock() == 0:
                                ...
                                # Done is True
                                if opt == 0:
                                    return reward, True, np.array(self.state_maker(self.line_state, self.timer_list, self.patterned_df, self.maxbuffer))
                                if opt == 1:
                                    return reward, True, np.array(self.state_maker_break(self.line_state, self.timer_list, self.patterned_df, self.maxbuffer))
                                if opt == 2:
                                    pass
                            else:
                                # Done is False
                                if opt == 0:
                                    return reward, False, np.array(self.state_maker(self.line_state, self.timer_list, self.patterned_df, self.maxbuffer))
                                if opt == 1:
                                    return reward, False, np.array(self.state_maker_break(self.line_state, self.timer_list, self.patterned_df, self.maxbuffer))
                                if opt == 2:
                                    pass
                    ...
                    # Blockage 발생 (후 버퍼의 용량이 남아있지 않아서 진행 불가능)
                        ...
                        self.blockage_time[machine_idx] += 1
                        self.line_state[0][machine_idx][0][4] += 1
                ...
                # Machine의 제품의 동작이 끝나지 않은 경우 + Machine상태가 DOWN인 경우
                    ...
                    self.timer_list[machine_idx][1] -= 1
                    # Down time 1추가
                    self.down_time[machine_idx] += 1
                    
                    # corrections 1
                    self.line_state[0][machine_idx][0][2] += 1
        ...
        if opt == 0:
            return 'A', False, np.array(self.state_maker(self.line_state, self.timer_list, self.patterned_df, self.maxbuffer))
        else:
            return 'A', False, np.array(self.state_maker_break(self.line_state, self.timer_list, self.patterned_df, self.maxbuffer))
        
    # 학습에 사용될 state를 생성 : break down 
    def state_maker_break(self, line_state, time_state, pattern_df, max_buffer): 
        # 결과를 저장할 list
        state = []
        # Machine state 확인
        idx = 0
